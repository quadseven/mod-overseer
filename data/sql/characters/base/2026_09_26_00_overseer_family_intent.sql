...
-- `on_the_table`, `changes` and `module_at` columns on every party poll (30 s)
-- with INSERT ... ON DUPLICATE KEY UPDATE of those columns only. The bridge
-- writes the `chosen_*` columns with an UPDATE of those columns only. Neither
-- side ever writes the other's, so neither can erase the other's answer.
--
