...
// The bag handed to a roster character with an empty bag slot (infra#2813).
//
// This is upstream's own choice, not ours: PlayerbotFactory::InitBags
// hardcodes 51809 (PlayerbotFactory.cpp:2625), so it is what every random bot
// on this realm is already carrying. Verified against item_template:
//
//     51809  "Portable Hole"  class 1 subclass 0  ContainerSlots 24
//            Quality 4  RequiredLevel 0
//
// A 24-slot epic bag on a level 11 character looks odd, and it is deliberately
// NOT special-cased to something more modest: matching what the rest of the
// realm's bots hold means one behaviour to reason about instead of two, and
// RequiredLevel 0 means it is legal for them to carry it. The name is repeated
// here only so that a reader of this module does not have to go and look it
// up; InitBags remains the thing that actually equips it.
constexpr uint32 OVERSEER_BAG_ITEM = 51809;
...
private:
    ...
    // How many of this character's four bag slots hold nothing at all.
    //
    // Four slots, four pointer reads, no allocation - cheap enough to ask on
    // every roster poll, and the point of asking is that the answer becomes
    // zero once the bags are handed out. Without it, GiveBags would construct
    // a PlayerbotFactory and walk four slots every twenty seconds for the rest
    // of the realm's life to do nothing.
    static uint32 CountEmptyBagSlots(Player* bot)
    {
        uint32 empty = 0;
    ...
    // Take a starter bag out of a bag slot, but ONLY while it is empty.
    //
    // WHY THIS IS NEEDED AT ALL, AND WHO IT IS FOR. Filling empty bag slots
    // alone helps four of the five and misses the one that matters most.
    // Measured live from character_inventory, bag slots 19-22:
    //
    //     who    bags equipped   empty bag slots   total slots
    //     Bork         1               3               22
    //     Grog         1               3               22
    //     Og           1               3               22
    //     Ugga         3               1               34
    //     Grug         4               0               40
    //
    // Grug carries FOUR Small Pouches and has no empty bag slot. He is also the
    // party leader, the only character carrying `new rpg`, and the one who sat
    // 2.98 yards from a Silverleaf node for 8h31m. A fix that silently skips
    // him is how this issue gets closed and stays broken.
    //
    // THE ONE RULE. Replacing a bag means destroying the bag that is there, and
    // `Player::DestroyItem` recursively destroys a bag's CONTENTS on the way
    // out (`if (pItem->IsNotEmptyBag())`, PlayerStorage.cpp:3132-3134). So this
    // refuses any bag holding anything whatsoever. `Bag::IsEmpty()` (Bag.h:46,
    // Bag.cpp:179-186) walks every slot in the container and is the only thing
    // standing between this function and a Westfall Deed. There is no quality
    // test, no item list and no exception: a bag with one copper coin in it is
    // left alone.
    //
    // WHY THE EQUIP IS PROVEN FIRST. If the replacement could not be equipped
    // after the old bag was already gone, the character would simply have lost
    // bag space. `Player::CanEquipNewItem` (Player.h:1317,
    // PlayerStorage.cpp:1885-1898) makes and deletes its own probe item, so
    // asking costs nothing. It is used instead of upstream's equivalent
    // because `PlayerbotFactory::CanEquipUnseenItem` is private
    // (PlayerbotFactory.h:159).
    //
    // WHY NOT JUST InitBags(true). Besides destroying occupied bags, upstream's
    // destroyOld branch is broken: after the DestroyItem at
    // PlayerbotFactory.cpp:2634-2635 it reaches `if (old_bag) continue;` at
    // :2637-2638 and never equips the replacement, leaving the slot empty. It
    // would take the bag and give nothing back.
    //
    // `blocked` counts the bags refused because they still hold something, so
    // the caller can say out loud that a character gained nothing and why.
    uint32 ReplaceEmptyStarterBags(Player* bot, uint32& blocked)
    {
        uint32 opened = 0;
        blocked = 0;
        ...
        for (uint8 slot = INVENTORY_SLOT_BAG_START; slot < INVENTORY_SLOT_BAG_END; ++slot)
        {
            ...
            if (!bag)
                continue;  // Already empty - InitBags fills this one.

            // Already the bag we would put there. Without this the pass
            // destroys and re-equips the same four bags on every poll.
            if (bag->GetEntry() == OVERSEER_BAG_ITEM)
                continue;
            ...
            uint16 dest = 0;
            if (bot->CanEquipNewItem(slot, dest, OVERSEER_BAG_ITEM, true) != EQUIP_ERR_OK)
            {
                ++blocked;
                continue;
            }
    ...
    // Give the roster the bags every random bot on this realm already has.
    //
    // WHY THEY HAD NONE (infra#2813). Measured live 2026-08-24: Bork, Grog and
    // Og were on 22 slots, Ugga 34, Grug 40, and every single one had ZERO
    // free. 22 is a 16-slot backpack plus one starter bag, with three of the
    // four bag slots simply empty - see the table on ReplaceEmptyStarterBags
    // for who has what. Upstream fills those slots in
    // PlayerbotFactory::InitBags, and the only caller is Randomize()
    // (PlayerbotFactory.cpp:759), which is RandomPlayerbotMgr's path. These
    // five fail IsRandomBot() three separate ways - named accounts cannot enter
    // the "<prefix>0..N" list, they are kept out of currentBots so they are
    // never re-rolled, and SelfBotLevel is 3 - so the bags were never coming.
    // Same wall as #2756 (talents), #2757 (professions) and #2782; this is the
    // fourth thing it has silently withheld.
    //
    // WHY BAGS AND NOT A VENDOR RULE. `sell vendor` ("everything worth
    // vendoring, not only greys") is the obvious move and it is the one that
    // would destroy the errand this issue is blocking. Only FOUR grey items
    // exist across all five characters - the bags are full of quality-1 WHITES,
    // and so are the live quest items: Westfall Deed, Gold Dust, Torn Murloc
    // Fin, Kobold Excavation Pick, and the Large Candles for quest 60, which is
    // COMPLETE and waiting to be handed in. Nothing here sells or trades
    // anything, so there is no exclusion list to get wrong.
    //
    // THE ONE THING THAT IS DESTROYED, STATED PLAINLY: a starter bag that
    // `Bag::IsEmpty()` has proven holds nothing, so that a real bag can go in
    // its place. That is the whole of it. No item in a bag is ever touched -
    // see ReplaceEmptyStarterBags for the guard and why it has no exceptions.
    //
    // WHY destroyOld=false, EXPLICITLY. The default is TRUE
    // (PlayerbotFactory.h:87), and in that branch InitBags calls
    // `bot->DestroyItem(INVENTORY_SLOT_BAG_0, slot, true)` on an OCCUPIED bag
    // slot (PlayerbotFactory.cpp:2634-2635) - which destroys the bag and
    // everything inside it, with no emptiness check of any kind.
    // `InitBags()` written bare is how a Large Candle dies. With false it only
    // ever equips into a slot that is already empty (the
    // `if (old_bag) continue;` at :2637-2638), so it cannot touch an item that
    // exists.
    //
    // WHAT THIS DOES NOT FIX. A character whose four bag slots all hold bags
    // that still have items in them gains NOTHING, because the only way to
    // upgrade a bag in place is to destroy the one there and that is refused
    // while it holds anything. On the measured roster that is Grug, and it is
    // logged loudly rather than skipped quietly. It also does not stop the bags
    // refilling: the family are all herbalists gathering into an alchemy sink
    // that IsRandomBot() also walled off (#2757), so this buys room, it does
    // not close the loop.
    void GiveBags(Player* bot, std::string const& name)
    {
        uint32 blocked = 0;
        uint32 const replaced = ReplaceEmptyStarterBags(bot, blocked);

        uint32 const empty = CountEmptyBagSlots(bot);
        if (!empty)
        {
            // NAMED, and at a level somebody reads. A character that gains no
            // slots is the interesting case, not the boring one: it is the
            // state Grug is in, and a silent skip on the one character with a
            // documented 8h31m freeze is how this gets marked fixed and stays
            // broken. It repeats every poll on purpose - the condition is not
            // self-correcting, and it clears the moment a bag empties.
            if (blocked)
                LOG_WARN("module.overseer",
                         "overseer: '{}' gains NO bag slots - all {} of its bag slots hold "
                         "bags that still contain items, and a bag can only be replaced "
                         "while it is empty. It keeps its starter bags until something in "
                         "one of them is used, handed in or moved.",
                         name, blocked);
            return;
        }

        LOG_INFO("module.overseer",
                 "overseer: giving '{}' bags for {} empty bag slot(s) ({} empty starter "
                 "bag(s) taken out, {} left alone because they still hold items)",
                 name, empty, replaced, blocked);

        PlayerbotFactory factory(bot, bot->GetLevel());
        factory.InitBags(false);

        // The `sell junk` shape: reports success, does nothing. If the bag item
        // cannot be created or equipped, InitBags takes its `continue` and
        // returns silently, and nobody would ever find out.
        if (uint32 const still = CountEmptyBagSlots(bot))
            LOG_ERROR("module.overseer",
                      "overseer: '{}' still has {} empty bag slot(s) after InitBags - "
                      "item {} could not be equipped", name, still, OVERSEER_BAG_ITEM);
    }
    ...
    void TrainRoster()
    {
        ...
        do
        {
            ...
            // BEFORE the level gate below, on purpose. Every one of these five
            // has been sitting at the same level for hours with full bags, so
            // behind that gate this call would compile, read correctly, and
            // never once run - the `sell junk` shape (reports success, does
            // nothing) that this epic keeps rediscovering. GiveBags does its
            // own cheap check and is a no-op once the slots are filled.
            GiveBags(bot, name);
